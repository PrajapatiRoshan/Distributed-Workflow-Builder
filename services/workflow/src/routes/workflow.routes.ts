...
workflowRouter.post('/validate', async (req, res, next) => {
    try {
...
workflowRouter.put('/:id', async (req, res, next) => {
    try {
...
workflowRouter.post('/:id/publish', requireRole('admin', 'user'), async (req, res, next) => {
    try {
        await db.query(
